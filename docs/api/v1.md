...
`events.sinks` is an optional provider-neutral registry. Configure signed sinks with `signedOutboundEventSink`; delivery remains explicit and cursor-driven so the embedding product chooses its worker, queue, cron, or workflow runtime.
...
`chats.import` accepts a typed UTF-8 source-file list, ZIP bytes, or a provider-neutral source adapter. It creates the chat and its first immutable version atomically without invoking the model. Archive imports reject unsafe or duplicate paths, encrypted and ZIP64 entries, symbolic links, binary files, oversized inputs, and excessive expanded size. `filePolicy.locked` accepts `"all"` or a path list; file-list imports may additionally set `locked: true` per file. Policy paths are normalized and must exist in the imported tree.
...
`files` returns paths sorted lexicographically. Each file contains `path`, `content`, `mediaType`, byte `size`, a SHA-256 `checksum`, and `locked`. Once a file is locked, direct changes, generated changes, and workspace tools cannot write, delete, or move it. New files are unlocked unless the import policy locks them.
...
`signedOutboundEventSink({ id, secret, keyId?, source?, send })` adapts the durable generation event log to an application-owned transport. `generation.deliverEvents({ sink, after, limit })` reads one durable page, sends it in cursor order, and returns the last successful cursor. The host persists one cursor per generation and sink. Delivery is at least once; the stable `<generationId>:<cursor>` event ID is the receiver's idempotency key.
...
Sink or transport errors never change the persisted generation. `OutboundEventDeliveryError` identifies the sink, failed event/cursor, and `lastDeliveredCursor`; retry from that last cursor. Because an endpoint may accept a request before the client observes a transport failure, receivers must still deduplicate stable event IDs. The sink contract assumes no HTTP client, queue vendor, or deployment runtime.
...
| `viby.version_files` | complete files and immutable lock metadata for one version | successful completion |
