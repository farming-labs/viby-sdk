---
title: "Complete v1 contract"
description: "The normative Viby lifecycle, persistence, ownership, and provider-neutral behavior contract."
---
...
method-oriented reference, start with the [SDK reference](/docs/api).
...
- sandbox and browser infrastructure, artifact-store provisioning, HTTP hosting, integration-app credentials, and redirect routes.

Viby owns:

- tenant- and user-scoped chat history;
- durable logical generations, immutable attempts, ordered events, and usage;
- immutable message feedback attributed to exact attempts, model identities, and versions;
...
interface VibyConfig<Framework extends string> {
  ...
  model?: LanguageModel; // convenient built-in Viby harness path
  ...
  generation?: {
    ...
    limits?: AgentRunnerConfig;
    engine?: GenerationEngine<Framework>; // mutually exclusive with model
    engines?: Readonly<Record<string, GenerationEngine<Framework>>>;
  ...
  events?: {
    sinks?: readonly OutboundEventSink[];
...
`generation.engine` is the provider-neutral escape hatch for a host that intentionally replaces
Viby's built-in model loop with another model runtime, agent harness, remote worker, or
orchestrator. `generation.engines` adds stable request-selectable aliases. Exactly one default
execution path is configured: either `model` or `generation.engine`. The former top-level `engine`
and `engines` fields remain deprecated compatibility aliases. Built-in AI SDK runner limits live
under `generation.limits`; the old model-only top-level `agent: { ...limits }` shape remains a
deprecated compatibility alias and does not represent another harness.

Every engine declares stable provider/model identity and optional capabilities for `change` and
`inspect` operations, streaming text, durable steering, typed traces, durable tool calls, and
binary artifacts. Engines receive a stable tenant/user/chat/generation/attempt run identity for
remote idempotency and may implement `close()` for client or session-pool cleanup. Viby rejects an
unsupported operation before invoking the engine and continues to own attempts, events, tasks,
permissions, quality gates, previews, immutable versions, and downloads.

`defineRemoteGenerationEngine` adapts an asynchronous provider harness through `start`, resumable
`events`, and optional `cancel` hooks. Starting is idempotent on the stable Viby attempt ID. Event
cursors are opaque, unique within the provider run, and never interpreted by Viby; deltas are copied
onto the normal durable Viby event stream before the remote run returns its final typed output.
The run identity and latest provider cursor are stored as an attempt-scoped engine checkpoint.
Checkpoint reads survive worker reclamation, while writes and cleanup require the current fenced
worker lease. Custom engines receive the same `context.checkpoint.load/save/clear` API.

Engines advertising `toolCalls: true` receive a provider-neutral `context.tools` catalog. Listing
returns host-selected JSON Schema definitions; invocation reuses the configured tool-source policy,
tenant context, secret isolation, durable call records, and idempotency keys. Approval-required
effects pause as ordinary Viby permission tasks and resume only after an exact allow decision.

### Read-only source inspection

`version.startInspection(input)` starts an addressable durable inspection of that exact immutable
version; `version.inspect(input)` waits and returns its persisted assistant `MessageData`.
`chat.startInspection(input)` and `chat.inspect(input)` target the latest version and reject chats
that do not yet have source. Successful `Generation.wait()` results use
`{ status: "responded", generation, message }` rather than pretending an inspection created a
version.

Inspection is enforced at several boundaries: only read/search workspace tools are exposed,
effectful inbound tools and sandbox commands are withheld, preview and quality-build work is
skipped, and the repository rejects any attempt to commit source from an inspection. The durable
generation configuration records `operation: "inspect"`; attempts, events, usage, cancellation,
retry, resume, and response messages use the normal generation lifecycle.
...
`events.webhooks` enables tenant-managed signed endpoints. `user.webhooks.create(...)` returns a
generated signing secret exactly once; configuration records never include secret bytes or their
store reference. `viby.webhookWorker({ id })` discovers due work across tenant scopes and can run
continuously or once from a host-owned process, cron, or workflow. Viby persists the endpoint start
cursor, filtering progress, leased attempts, retry state, dead letters, and redrive state across
restarts. `user.webhooks.deliver(webhookId, generationId)` remains available for explicit drains.
...
interface Chat<Framework extends string> {
  ...
  submitFeedback(messageId: string, input: SubmitMessageFeedbackInput): Promise<MessageFeedbackData>;
  listFeedback(messageId: string): Promise<readonly MessageFeedbackData[]>;
  getSelectedFeedback(messageId: string): Promise<MessageFeedbackData | null>;
...
`ScopedViby.feedback.analytics(...)` aggregates immutable message feedback by model, engine/runtime
alias, exact skill set, framework, generation version, or a compound set of those dimensions. New
feedback records atomically update the user's selected rating pointer, while idempotent replay never
rewinds a newer selection. The Web API returns both `feedback` history and `selected` from the
message route and exposes scoped analytics at `GET /feedback/analytics`.
...
Paginated methods default to 20 and accept 1–100. Each page contains `items` and an opaque `nextCursor`; pass that value back as `after`. Chat records are ordered by most recently updated, versions by descending number, and messages by a strictly increasing per-chat creation timestamp. This preserves deterministic conversation and cursor order even when a prompt and response reach storage within the same clock millisecond. `chat.getMessage(id)` returns one message with its complete ordered parts and rejects ids from another chat, tenant, or user as not found. `chats.list({ metadata })` applies tenant-scoped JSON containment before pagination, including nested objects and array members; reuse the same filter with subsequent cursors. A cursor is resource-specific and malformed or cross-resource cursors throw `ConfigurationError`.
...
`chat.enqueue({ prompt, afterGenerationId, ... })` creates a normal durable generation whose
`afterGenerationId` points to a generation in the same scoped chat. Its user message and initial
attempt are visible immediately, but workers cannot claim it until the predecessor succeeds and has
an immutable version. Claiming atomically sets that version as `baseVersionId`. Chained follow-ups
therefore preserve exact source lineage across reloads and process restarts. `chat.queuedGenerations()`
lists waiting follow-ups. A failed or cancelled predecessor leaves dependents queued for an explicit
retry or cancellation rather than silently skipping work.
...
interface Generation<Framework extends string> {
  ...
  providerRequests(): Promise<ProviderRequestAttributionData[]>;
...
Configure provider-neutral quality gates when generated source must install and
build before it becomes an immutable version:
...
Generation engines can additionally report every provider call through
`context.attribution.record(...)`. Each immutable record is owned by one attempt and stores a stable
idempotency key, optional provider request ID, actual routed provider/model, outcome, token and
cache usage, latency, an engine-reported cost estimate, and credential-free model metadata. Query
the ordered history through `generation.providerRequests()` or
`GET /generations/{generationId}/provider-requests`; retries never overwrite earlier calls.
...
| `viby.generation_engine_checkpoints` | credential-free opaque engine state, revision, and remote cursor scoped to one attempt | active worker checkpoint save or terminal cleanup |
...
- `ToolSourceAuthorizationError`: a tool-source provider authorization request or callback failed;
- `ToolSourceConnectionRequiredError`: a selected durable tool source has no usable connection;
...
- SDK-owned preview hosting, managed provider-app registration, or host-owned provider credentials.
