...
Viby materializes the complete candidate source in a fresh ephemeral sandbox,
runs preparation and checks in order, and commits the version only after every
command exits with code `0`. Failed checks fail the attempt without persisting a
partial version. `quality.started` and `quality.completed` use the existing
resumable event cursor; command output is intentionally not persisted because it
may contain secrets. The configured sandbox command policy applies to every
quality command. Quality policy is host configuration and must be identical on
all durable workers.
