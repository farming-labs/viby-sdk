...
interface VibyConfig<Framework extends string> {
  ...
  model: LanguageModel; // or agent: CodingAgent<Framework>, or engine: GenerationEngine<Framework>
  models?: Readonly<Record<string, LanguageModel>>;
  agents?: Readonly<Record<string, CodingAgent<Framework>>>;
  storage?: {
...
`agent` is the equivalent ergonomic path for a packaged `CodingAgent`. For example,
`codex({ model })` from `@viby/sdk/agent/codex` uses the official Codex SDK while Viby keeps
ownership of durable attempts, events, messages, usage, and immutable versions. `agents` adds
stable request-selectable aliases. `engine` remains the lowest-level provider-neutral escape hatch
for custom model runtimes and orchestrators. Exactly one of `model`, `agent`, or `engine` is
configured. Built-in AI SDK runner limits live under `generation.limits`; the old model-only
top-level `agent: { ...limits }` shape remains a deprecated compatibility alias.
