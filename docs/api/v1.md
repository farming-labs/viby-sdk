...
store reference. Delivery remains explicit through `user.webhooks.deliver(webhookId, generationId)`
so the host owns scheduling. Viby persists filtering progress, leased attempts, retry state, dead
letters, and redrive state across restarts.
