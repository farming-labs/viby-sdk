...
export class MemoryRepository implements Repository {
  ...
  /** Internal durable snapshot used by the SQLite adapter and published test fixtures. */
  exportState(): MemoryRepositoryState {
    const fields: Record<string, unknown> = {};
    for (const [key, value] of Object.entries(this)) {
      if (key !== "closed") fields[key] = value;
    }
    return { revision: 1, cursor: this.#cursor, fields };
  }

  /** Restore a snapshot produced by exportState(). */
  importState(state: MemoryRepositoryState): void {
    if (!state || state.revision !== 1 || !Number.isInteger(state.cursor) || !state.fields) {
